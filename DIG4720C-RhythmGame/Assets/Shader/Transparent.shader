Shader "Pat/Transparent"
{
    Properties
    {
		...
		_TimeSize("SizeOverTime", float) = 0
		 ...
		 _Transp("Transparent", Range (0, 1)) = 0.25
    }
    SubShader
    {
        Pass
        {
            ...
            v2f vert (appdata_base v)
            {
				...
				{
				v.vertex.xyz += v.normal.xyz * _ExtrudeAmt * (1 + (sin(_Time.y/_SizeSpeed)));
            ...
            fixed4 frag (v2f i) : SV_Target
            {
				...
				col.a = tex2D(_DisolveTex, i.uv) *_Transp;
                return col;
            }
            ENDCG
        }
